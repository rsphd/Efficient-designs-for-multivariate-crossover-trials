...
write_xlsx(data,"D:/Code for Manuscript 2 New 2/Mat_half_Mat_3half_p3t3.xlsx")

#Calling library readxl
library(readxl)

Mat_half_Mat_3half_p3t3=read_excel("D:/Code for Manuscript 2 New 2/Mat_half_Mat_3half_p3t3.xlsx")

#Plot for Minimum

plot(rho_11,Mat_half_Mat_3half_p3t3$Minimum_Orthogonal,ylim = range(c(Mat_half_Mat_3half_p3t3$Minimum_Orthogonal,100)),xlab=expression('r'[(1)]), ylab="Minimum relative difference (in %)",type="l",lty="solid",lwd=3)
par(new=TRUE)
lines(rho_11,Mat_half_Mat_3half_p3t3$Minimum_Uniform,lty="dotted",lwd=3)

legend(x = "right", lty = c("solid","dotted"), lwd=c(3,3), text.font = 4, col= c("black"),text.col = "black", legend=c(expression('d'^"*"),expression('d'[1])))

#Plot for Maximum

plot(rho_11,Mat_half_Mat_3half_p3t3$Maximum_Orthogonal,ylim = range(c(Mat_half_Mat_3half_p3t3$Maximum_Orthogonal,100)),xlab=expression('r'[(1)]), ylab="Maximum relative difference (in %)",type="l",lty="solid",lwd=3)
par(new=TRUE)
lines(rho_11,Mat_half_Mat_3half_p3t3$Maximum_Uniform,lty="dotted",lwd=3)
